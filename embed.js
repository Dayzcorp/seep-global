(function(){
  ...
  function init(c){
    ...
    try{document.documentElement.style.setProperty('--seep-color',color);}catch(e){}
    css('#seep-bubble{position:fixed;top:50%;right:20px;width:60px;height:60px;margin-top:-30px;border-radius:30px;background:var(--seep-color,'+color+');color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 2px 6px rgba(0,0,0,.3);z-index:2147483647}#seep-container{position:fixed;top:50%;right:90px;transform:translateY(-50%);width:320px;max-width:90vw;height:450px;max-height:70vh;background:#fff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.2);display:none;flex-direction:column;overflow:hidden;z-index:2147483647}#seep-header{background:var(--seep-color,'+color+');color:#fff;padding:10px;font-size:16px}#seep-messages{flex:1;overflow-y:auto;padding:10px;font-size:14px}#seep-input{display:flex;border-top:1px solid #eee}#seep-input textarea{flex:1;padding:8px;border:none;resize:none;font-size:14px}#seep-input button{background:var(--seep-color,'+color+');color:#fff;border:none;padding:0 12px;cursor:pointer}.seep-msg{margin-bottom:8px}.seep-msg.bot{background:#f1f5ff;padding:6px;border-radius:4px}.seep-msg.user{text-align:right}.seep-btn{display:inline-block;margin:4px 4px 0 0;padding:6px 10px;background:var(--seep-color,'+color+');color:#fff;border-radius:4px;text-decoration:none;cursor:pointer}.seep-btn:hover{opacity:.8}#seep-hint{font-size:12px;color:#666;padding:4px;display:none}');
    ...
    f.innerHTML='<div id="seep-header">Ask Seep</div><div id="seep-messages"></div><div id="seep-input"><textarea rows="1"></textarea><button>Send</button></div><div id="seep-hint">Try asking about shipping, returns, or order status.</div>';
